...
Page({
  ...
  repealBtn:function(e){
    wx.showModal({
      title: '提示',
      content: '本次申请撤销后，你仅能再发起一次申请，请您确认是否撤销申请？',
      success: function (res) {
        if (res.confirm) {
          console.log('用户点击确定')
          wx.showLoading({
          ...
          wx.request({
            url: 'http://www.paokucoin.com/app/ewei_shopv2_api.php?i=1&r=group.order.cancel_refund&comefrom=wxapp',
            data:{
              openid:useropenid,
              refund_id:e.currentTarget.dataset.id
            },
            success:function(res){
              console.log(res);
              if(res.data.error == 0){
  ...
  onShareAppMessage: function (ops) {
    ...
    if (ops.from === 'button') {
      ...
      return {
        title: '仅剩'+num+'名额',
        path: '/packageA/pages/changce/spelltuan/details/message?team_id=' + ops.target.dataset.teamid + '&ball=' + ops.target.dataset.ball+'&id=' + ops.target.dataset.goodid+'&xuan=0',
